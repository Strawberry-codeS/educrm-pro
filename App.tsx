...
const App: React.FC = () => {
  ...
  const renderView = () => {
    switch (currentView) {
      case ViewType.WORKTABLE:
        return <Worktable onNavigate={navigateTo} />;
      ...
      case ViewType.EDIT_INFO:
        return selectedStudent ? (
          <EditInfo student={selectedStudent} onBack={goBack} />
        ) : null;
      default:
        return <Worktable onNavigate={navigateTo} />;
    }
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-background-light dark:bg-slate-950">
      <div className="flex-1 overflow-y-auto hide-scrollbar pb-24">
        {renderView()}
      </div>

      {isMainTab && (
        <nav className="fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-t border-slate-100 dark:border-slate-800 px-10 py-3 pb-8 flex justify-between items-center z-50">
          <button 
